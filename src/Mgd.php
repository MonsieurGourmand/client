...
class Mgd {
    ...
    public function post($url, $object, $entityClass) {
        $response = $this->client->fetch($this->apiUrl . $url . '.json',array(),$this->serializer->serialize($object),\OAuth2\Client::HTTP_METHOD_POST);
        if(self::getError($response))
    ...
    public function put($url, $id, $object, $entityClass) {
        ...
        $response = $this->client->fetch($this->apiUrl . $url .'/'.$id. '.json',array(),$object,\OAuth2\Client::HTTP_METHOD_PUT,array('Content-Type: application/json','Content-Length:'.strlen($object)));
        if(self::getError($response))
